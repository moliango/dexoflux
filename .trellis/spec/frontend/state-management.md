...
- Reading settings must expose only settings that are wired to real behavior. `AppSettings.openExternalLinksInAppBrowser` controls whether Topic Detail, Replies, Me, and profile summary links open http(s) URLs in `InAppBrowserViewController` (via `DexoSafariPresenter`) or hand them to `UIApplication.open(...)`. Non-web schemes always go to the system. `AppSettings.defaultExpandRelatedLinks` controls the initial expanded state of Topic Detail related-link cards.
